%Length scale computations with seasonal dependence
clear;
...
for i = 1:length(tag_names)
    ...
    if exist(varname, 'var')
        ...
        source_length_scale(i)=mean(sqrt(UQ(x,y,:).^2+VQ(x,y,:).^2),3)./mean(P(x,y,:),3)./1000;
        sink_length_scale(i)=mean(sqrt(UQ(x,y,:).^2+VQ(x,y,:).^2),3)./mean(QFLX(x,y,:),3)./1000;
...
for i = 1:length(tag_names)
    fprintf('PRECT_pnt%s: %.2f km\n', tag_names{i}, source_length_scale(i));
    fprintf('PRECT_pnt%s: %.2f km\n', tag_names{i}, sink_length_scale(i));
...
T = table(tag_names', ...
          source_length_scale', ...
          sink_length_scale', ...
          'VariableNames', {'Tag', 'source_length_scale','sink_length_scale'});

% === Write to Excel ===
filename = 'winter_length_scales.xlsx';  % Creates a new file or overwrites
