%Compare evaporation fractions to PRECTocn/ocean_mask fractions
%CESM averages
% Load the data
...
weighted_avg_QFLX = sum(QFLX_filtered.*P_filtered, 3, 'omitnan') ./ sum(P_filtered, 3, 'omitnan');
...
figure;
subplot(1, 1, 1);
make_figure_pcolor(((weighted_avg_QFLX.*ocean_mask)./weighted_avg_QFLX), lat, lon, [-90 90], [-180 180], linspace(-60, 60, 100), 1);
clim([0 1]);
xlabel('Longitude');
ylabel('Latitude');
title('Weighted Average of ocean fraction (YEARLY)');

dummy2=sum(sum(sum(QFLX_filtered.*ocean_mask,3).*cosd(lat'), 'omitnan'), 'omitnan')./sum(sum(sum(QFLX_filtered,3).*cosd(lat'),'omitnan'), 'omitnan');

display(['average of (evapfrac): ',num2str(dummy2)])